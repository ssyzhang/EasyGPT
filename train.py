...
# I/O
    ...
    # dropout: float = 0.0
    # use_bias: bool = False # True: bias in Linears and LayerNorms, like GPT-2. False: a bit better and faster
...
# data
dataset = 'openwebtext' # 'openwebtext' or 'shakespeare_char' or etc.
...
def print_gpu_memory():
    ...
    # 1. Allocated: 实际被 Tensor 占用的显存（你的模型权重、梯度、激活值）
    allocated = torch.cuda.memory_allocated() / GB
    
    # 2. Reserved: PyTorch 向系统申请的总显存（包含 allocated + 碎片/未使用的缓存）
    # 这才是你在任务管理器或 nvidia-smi 里看到的数值
    reserved = torch.cuda.memory_reserved() / GB
    
    # 3. Peak: 从程序开始（或上次重置）到现在的最大占用峰值
...
# model init
model_args = dict(num_hidden_layers=num_hidden_layers, 
...
if max_position_embeddings < model.config.block_size:
    model.crop_block_size(max_position_embeddings)
    model_args['block_size'] = max_position_embeddings # so that the checkpoint will have the right value
model.to(device)
